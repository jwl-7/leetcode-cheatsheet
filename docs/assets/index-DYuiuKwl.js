...
\r
    return distances\r
`,zv=`def kruskal_mst(n: int, edges: list[tuple[int, int, int]]) -> list[tuple[int, int, int]]:\r
    mst = []\r
    uf = UnionFind(n)\r
    edges.sort()\r
\r
...
\r
    return mst\r
`,Fv=`from heapq import heappop\r
\r
\r
...
\r
    return mst\r
`,Uv=`from collections import defaultdict, deque\r
\r
\r
...
`,Vv=`/*\r
    ...
    Try using a language that doesn't hate you, like Python.\r
*/\r
`,Kv=`const kahnTopologicalSort = (graph) => {\r
    const result = []\r
    const queue = []\r
